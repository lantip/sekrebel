...
function countScore(item, value) {
    ...
    let nilai = localStorage.getItem("score-kewan");
    let koreksi = localStorage.getItem("koreksi-kewan");
    let sumnil = {};
    let koreksis = [];
    if (koreksi) {
    ...
    localStorage.setItem("score-kewan", JSON.stringify(sumnil));
    localStorage.setItem("koreksi-kewan", JSON.stringify(koreksis));
...
const writeToPage = () => {
    ...
    nextBtn.addEventListener('click',function(){
        ...
        if(i < wordDetails.length){
            hint.src = "gambar/"+wordDetails[i].hint;
...
function checkAnswer(){
    ...
    if(isCorrect === true){
        ...
        get('.showHint').addEventListener('click', function(){
            hintAnswer.innerHTML = `<div style="padding: 10px 15px; text-align: left">Iki arané kéwan ${wordDetails[currentArrayIndex].hint.replace("__","/").replace("--", " ").replace(".png","")} ( <span class="hintJawa">${wordDetails[currentArrayIndex].word.join('')}</span> ). <br>Cara nulisé yaiku: <span class="hintJawa">${wordDetails[currentArrayIndex].word.join('&#x200C; + ‌&#x200C;')}</span></div>`
